...
| **uav-platform-service** | 8080 | 平台编排，服务间编排调用链，数据源CRUD，实时数据获取，不含独立认证/路径规划逻辑 |
...
**ADR-002**: 所有公共工具和组件统一收归 common-utils。SecurityAuditor 为唯一审计实现，各模块通过 SecurityAuditConfig 委托调用。
