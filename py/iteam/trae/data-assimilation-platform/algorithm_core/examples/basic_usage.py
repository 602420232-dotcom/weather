...
def generate_background_field_chunked(grid_shape, domain_size):
    """分块生成背景风场，避免大数组一次性加载"""
    nx, ny, nz = grid_shape
...
def generate_background_field(grid_shape, domain_size):
    """生成物理合理的背景风场 - 内存优化版本"""
    nx, ny, nz = grid_shape
...
def adaptive_resolution_assimilation(assimilator, config, min_resolution=5.0):
    """动态分辨率同化，根据计算资源自动调整"""
...
def generate_multi_variable_background(grid_shape, domain_size):
    """生成多变量背景场（风速、温度、湿度）"""
    nx, ny, nz = grid_shape
    x, y, z = np.meshgrid(
        np.linspace(0, domain_size[0], nx),
...
def main():
    ...
    try:
        ...
        print(f"温度场范围: [{multi_var_background['temperature'].min():.2f}, {multi_var_background['temperature'].max():.2f}] °C")
        print(f"湿度场范围: [{multi_var_background['humidity'].min():.2f}, {multi_var_background['humidity'].max():.2f}] %")
