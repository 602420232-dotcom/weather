class BlockDecomposition:
    def __init__(self, config=None):
        self.config = config
    def decompose(self, grid_shape):
        # 返回一个包含单一块的列表
        return [{'indices': (slice(0, grid_shape[0]), slice(0, grid_shape[1]), slice(0, grid_shape[2])), 'offset': (0, 0, 0), 'shape': grid_shape}]
    def process_parallel(self, blocks, func):
        # 顺序处理所有块
        return [func(block) for block in blocks]
    def merge_blocks(self, blocks, results, grid_shape):
        # 简单合并，假设只有一个块
        return results[0]
"""
基础使用示例
