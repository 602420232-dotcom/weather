...
class FourDimensionalVar(AssimilationBase):
    ...
    def cost_function(self, x0_flat: np.ndarray, xb: Dict[str, np.ndarray], 
        ...
        shape = xb['T'].shape
        x0 = {
            'T': x0_flat[:shape[0]*shape[1]].reshape(shape),
            'U': x0_flat[shape[0]*shape[1]:2*shape[0]*shape[1]].reshape(shape),
            'V': x0_flat[2*shape[0]*shape[1]:3*shape[0]*shape[1]].reshape(shape),
            'Ps': x0_flat[3*shape[0]*shape[1]:].reshape(shape)
        }
        ...
        for k in reversed(range(len(model_time_series))):
            ...
            for obs in observations:
                if obs.get('time_idx', 0) == k:
                    ...
                    adj_obs = R_inv @ diff
                    # 观测算子H的伴随
                    lambda_T += adj_obs * 1.0
                    lambda_U += adj_obs * 0.2
                    lambda_V += adj_obs * 0.2
    ...
    def _apply_B_inv(self, x: np.ndarray, B_inv: np.ndarray) -> np.ndarray:
        ...
        """
        if B_inv.ndim == 2:
            return B_inv @ x.ravel().reshape(-1, 1).ravel()
        else:
            return x * B_inv
    ...
    def assimilate(self, background: Dict, observations: List[Dict]) -> Tuple[Dict, Dict]:
        ...
        # 准备背景场
        xb = {
            'T': background['variables'].get('temperature', np.zeros((10, 10))),
            'U': background['variables'].get('u_wind', np.zeros((10, 10))),
            'V': background['variables'].get('v_wind', np.zeros((10, 10))),
            'Ps': background['variables'].get('pressure', np.zeros((10, 10)))
        }
        ...
        shape = xb['T'].shape
        n = shape[0] * shape[1]
        B_inv = np.eye(n) * 0.1  # 简化的背景误差协方差逆矩阵
        R_inv = np.eye(len(observations)) * 1.0  # 简化的观测误差协方差逆矩阵
        ...
        # 最优分析场
        xa = {
            'T': result.x[:shape[0]*shape[1]].reshape(shape),
            'U': result.x[shape[0]*shape[1]:2*shape[0]*shape[1]].reshape(shape),
            'V': result.x[2*shape[0]*shape[1]:3*shape[0]*shape[1]].reshape(shape),
            'Ps': result.x[3*shape[0]*shape[1]:].reshape(shape)
        }
        ...
        try:
            # 简化的方差场计算
            n_total = len(x0_flat)
            H_mat = np.eye(n_total) * 0.1
            P = np.linalg.inv(B_inv + H_mat.T @ R_inv @ H_mat)
            variance_field = {
                'temperature': np.diag(P)[:n].reshape(shape),
                'u_wind': np.diag(P)[n:2*n].reshape(shape),
                'v_wind': np.diag(P)[2*n:3*n].reshape(shape),
                'pressure': np.diag(P)[3*n:].reshape(shape)
            }
        except Exception as e:
            logger.warning(f"方差场计算失败: {e}")
            # 生成默认方差场
            variance_field = {
                'temperature': np.ones(shape) * 0.5,
                'u_wind': np.ones(shape) * 0.5,
                'v_wind': np.ones(shape) * 0.5,
                'pressure': np.ones(shape) * 0.5
            }
