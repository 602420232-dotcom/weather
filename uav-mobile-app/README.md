# 无人机气象驱动智能路径规划系统 - 移动客户端

基于 Flutter 开发的跨平台移动应用，支持 iOS 和 Android。

## 功能特性

- 🏠 **系统驾驶舱** - 实时概览系统状态、任务统计、气象预警
- 🗺️ **路径规划** - 交互式地图展示，VRPTW/A*/DWA 三层路径规划
- 🌤️ **气象数据** - WRF气象数据可视化，风速风向热力图
- 📋 **任务管理** - 无人机任务创建、分配、执行、跟踪
- 🚁 **无人机管理** - 无人机注册、状态监控、电量管理
- 📊 **系统监控** - CPU/内存/磁盘资源监控，服务状态巡检
...
│   │   ├── data_source.dart            # 数据源模型
│   │   └── system_status.dart          # 系统状态模型
│   ├── services/
...
应用默认连接到 `http://localhost:8088` 的 API 网关，集成以下微服务接口：

| 服务 | 端点 | 用途 |
|------|------|------|
| Platform | `/api/v1/*` | 用户认证、无人机/任务管理 |
| WRF Processor | `/api/wrf/*` | WRF 气象数据解析 |
| Meteor Forecast | `/api/forecast/*` | 气象预测与订正 |
| Path Planning | `/api/planning/*` | VRPTW/A*/DWA 路径规划 |
| Data Assimilation | `/api/assimilation/*` | 贝叶斯数据同化 |
