...
class DERRTStarPlanner:
    ...
    def solve(self) -> dict[str, Any]:
        ...
        for _ in range(self.max_iterations):
            ...
            best_cost = costs[nearest_idx] + np.linalg.norm(np.array(nodes[nearest_idx]) - new_point)
            for i, node in enumerate(nodes[:-1]):
                c = costs[i] + np.linalg.norm(np.array(node) - new_point)
                if c < best_cost and not self._check_line_collision(node, new_point_tuple):
                    best_parent = i
                    best_cost = c
            parents[new_idx] = best_parent
            costs[new_idx] = best_cost
            if np.linalg.norm(new_point - np.array(self.goal)) < self.goal_radius:
                path = self._extract_path(new_idx, nodes, parents)
                return {"path": path, "cost": float(best_cost), "iterations": _ + 1}
    ...
    def _check_collision(self, point):
        for obs in self.obstacles:
            ...
            if np.linalg.norm(np.array(point) - np.array([obs[0], obs[1]])) < r:
                return True
        return False
