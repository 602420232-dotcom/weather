...
class CompositeRiskAssessor:
    ...
    def assess(self) -> dict[str, Any]:
        ...
        w = self.weights
        risk_score = w.get("weather", 0.4) * weather["risk_score"] + w.get("terrain", 0.3) * terrain["risk_score"] + w.get("airspace", 0.3) * airspace["risk_score"]
        risk_level = "low" if risk_score < 0.3 else "medium" if risk_score < 0.6 else "high"
        return {"risk_score": float(risk_score), "risk_level": risk_level, "breakdown": {"weather": weather, "terrain": terrain, "airspace": airspace}, "weights_used": self.weights}
