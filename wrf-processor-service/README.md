...
WRF 气象数据处理服务用于解析和处?WRFWeather Research and Forecasting模型输出的 NetCDF4 格式气象数据提取低空气象参数供路径规划使用?
...
| 接口 | 方法 | 说明 |
|------|------|------|
| `/api/wrf/parse` | POST | 解析 WRF 输出文件Multipart?|
...
- **格式**: NetCDF4 (.nc)
- **字段要求**: 需包含 temperaturehumiditywind_speedpressure 等气象变?
